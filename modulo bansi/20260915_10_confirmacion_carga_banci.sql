...
/* Ejecutar completo en SSMS contra DESARROLLO, sin modo SQLCMD.
   Incluye estructura incremental y ambos procedimientos en una sola transacción.
   No procesa, elimina, rechaza ni confirma cargas existentes.
   Después requiere la nueva API: /validar ya no podrá integrar automáticamente.
*/
IF DB_NAME() <> N'siiid2' THROW 52430, 'La base destino debe ser siiid2.', 1;
IF @@TRANCOUNT <> 0 THROW 52431, 'Use una ventana sin transacciones abiertas.', 1;
IF OBJECT_ID(N'dbo.banci_carga', N'U') IS NULL OR OBJECT_ID(N'dbo.banci_carga_observacion', N'U') IS NULL
    OR OBJECT_ID(N'dbo.banci_carga_tmp_carpeta', N'U') IS NULL OR OBJECT_ID(N'dbo.banci_carga_tmp_delito', N'U') IS NULL
    OR OBJECT_ID(N'dbo.banci_carga_tmp_victima', N'U') IS NULL OR OBJECT_ID(N'dbo.banci_historial_cambio', N'U') IS NULL
    OR OBJECT_ID(N'dbo.sp_banci_procesar_carga', N'P') IS NULL
    THROW 52432, 'Falta la estructura BANCI existente. Este es un script incremental.', 1;
IF COL_LENGTH(N'dbo.banci_victima', N'id_tv') IS NULL OR COL_LENGTH(N'dbo.banci_victima', N'id_tpm') IS NULL
    OR COL_LENGTH(N'dbo.banci_carga_tmp_victima', N'id_tv') IS NULL OR COL_LENGTH(N'dbo.banci_carga_tmp_victima', N'id_tpm') IS NULL
    OR COL_LENGTH(N'dbo.banci_victima', N'curp') < 100
    THROW 52433, 'Faltan ajustes anteriores BANCI (campos heredados o CURP de 50 caracteres). Aplique 03 a 09 según corresponda.', 1;
IF EXISTS (SELECT 1 FROM sys.columns WHERE object_id = OBJECT_ID(N'dbo.banci_victima') AND name = N'no_banci' AND (is_nullable = 0 OR default_object_id <> 0))
    OR EXISTS (SELECT 1 FROM sys.key_constraints WHERE parent_object_id = OBJECT_ID(N'dbo.banci_victima') AND name = N'UQ_banci_victima_no_banci')
    THROW 52434, 'Falta el ajuste previo de No_BANCI nullable y su índice filtrado (03/06).', 1;

BEGIN TRY
    BEGIN TRANSACTION;
    DECLARE @BloqueoInstalacion INT;
    EXEC @BloqueoInstalacion = sys.sp_getapplock @Resource = N'BANCI:INSTALACION:CONFIRMACION', @LockMode = N'Exclusive', @LockOwner = N'Transaction', @LockTimeout = 10000;
    IF @BloqueoInstalacion < 0 THROW 52435, 'Hay otra actualización del esquema BANCI en curso.', 1;

EXEC sys.sp_executesql N'IF COL_LENGTH(N''dbo.banci_carga'', N''aceptada_usuario'') IS NULL
    ALTER TABLE dbo.banci_carga ADD aceptada_usuario BIT NULL;
IF COL_LENGTH(N''dbo.banci_carga'', N''id_usuario_confirmacion'') IS NULL
    ALTER TABLE dbo.banci_carga ADD id_usuario_confirmacion INT NULL;
IF COL_LENGTH(N''dbo.banci_carga'', N''fecha_confirmacion'') IS NULL
    ALTER TABLE dbo.banci_carga ADD fecha_confirmacion DATETIME2(7) NULL;
';
EXEC sys.sp_executesql N'IF EXISTS (SELECT 1 FROM sys.check_constraints WHERE parent_object_id = OBJECT_ID(N''dbo.banci_carga'') AND name = N''CK_banci_carga_estado'')
    ALTER TABLE dbo.banci_carga DROP CONSTRAINT CK_banci_carga_estado;

ALTER TABLE dbo.banci_carga WITH CHECK ADD CONSTRAINT CK_banci_carga_estado
    CHECK (estado IN (N''RECIBIDO'', N''VALIDANDO'', N''VALIDADO'', N''VALIDADO_PENDIENTE'', N''PROCESANDO'', N''PROCESADO'', N''PROCESADO_CON_ADVERTENCIAS'', N''RECHAZADO_VALIDACION'', N''ERROR''));

IF NOT EXISTS (SELECT 1 FROM sys.foreign_keys WHERE parent_object_id = OBJECT_ID(N''dbo.banci_carga'') AND name = N''FK_banci_carga_confirmacion_usuario'')
    ALTER TABLE dbo.banci_carga WITH CHECK ADD CONSTRAINT FK_banci_carga_confirmacion_usuario
        FOREIGN KEY (id_usuario_confirmacion) REFERENCES dbo.usuario(id_usuario);

IF NOT EXISTS (SELECT 1 FROM sys.check_constraints WHERE parent_object_id = OBJECT_ID(N''dbo.banci_carga'') AND name = N''CK_banci_carga_decision'')
    ALTER TABLE dbo.banci_carga WITH CHECK ADD CONSTRAINT CK_banci_carga_decision
        CHECK
        (
...
IF OBJECT_ID(N''dbo.banci_carga_bitacora_estado'', N''U'') IS NULL
BEGIN
...
';
    ...
    COMMIT TRANSACTION;
    PRINT N'Confirmación BANCI instalada. No se modificaron datos de cargas existentes. Falta desplegar la API y el Front nuevos.';
END TRY
BEGIN CATCH
    IF XACT_STATE() <> 0 ROLLBACK TRANSACTION;
    THROW;
